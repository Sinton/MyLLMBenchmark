...
export function getReportKpis(detail: ReportDetail) {
  ...
  const common = [
    ...
    { label: "P95 延迟", helpKey: "p95", value: detail.summary.p95_latency_ms, unit: "ms", hint: "请求尾延迟" },
    { label: "稳定 QPS", helpKey: "qps", value: detail.stable_qps, unit: "req/s", hint: `成功率 ${detail.summary.success_rate}%` },
  ];

  if (modelType === "embedding") {
    return [
      ...common,
      { label: "Batch Size", helpKey: "batch", value: detail.workload_config.batch_size ?? 16, unit: "条/批", hint: "每个请求文本数量" },
      { label: "Text/s", helpKey: "text_s", value: latestStage(detail)?.text_count ?? "-", unit: "条/s", hint: "每秒处理文本条数" },
      { label: "Input Token/s", helpKey: "input_s", value: detail.input_tokens, unit: "token/s", hint: "向量化输入吞吐" },
  ...
  if (modelType === "rerank") {
    return [
      ...common,
      { label: "Docs/Query", helpKey: "docs_q", value: detail.workload_config.documents_per_query ?? 30, unit: "docs", hint: "每个 query 候选数" },
      { label: "Pair/s", helpKey: "pair_s", value: latestStage(detail)?.pair_count ?? "-", unit: "pair/s", hint: "query-doc 对吞吐" },
...
export function getStageColumns(modelTypeValue: string): StageColumn[] {
  ...
  const commonEnd: StageColumn[] = [
    { key: "p95", label: "P95", helpKey: "p95", render: (stage) => `${stage.p95_latency_ms}ms` },
    { key: "success", label: "成功率", helpKey: "success_rate", render: (stage) => `${stage.success_rate}%` },
  ];

  if (modelType === "embedding") {
    return [
      ...commonStart,
      { key: "qps", label: "QPS", helpKey: "qps", render: (stage) => stage.qps },
      { key: "batch", label: "Batch", helpKey: "batch", render: (stage) => stage.batch_size || "-" },
      { key: "text", label: "Text/s", helpKey: "text_s", render: (stage) => stage.text_count || "-" },
      { key: "input", label: "Input/s", helpKey: "input_s", render: (stage) => stage.input_tokens },
      ...commonEnd,
    ];
  }
  ...
  if (modelType === "rerank") {
    return [
      ...commonStart,
      { key: "qps", label: "Query/s", helpKey: "query_s", render: (stage) => stage.qps },
      { key: "docs", label: "Docs/Q", helpKey: "docs_q", render: (stage) => stage.documents_per_query || "-" },
      { key: "pair", label: "Pair/s", helpKey: "pair_s", render: (stage) => stage.pair_count || "-" },
      ...commonEnd,
    ];
  }
  ...
  if (modelType === "multimodal") {
    return [
      ...commonStart,
      { key: "qps", label: "QPS", helpKey: "qps", render: (stage) => stage.qps },
      { key: "image", label: "Image/s", helpKey: "image_s", render: (stage) => Math.round(stage.qps * Math.max(1, stage.image_count)) },
      { key: "ttft", label: "TTFT", helpKey: "ttft", render: (stage) => `${stage.ttft_ms}ms` },
      ...commonEnd,
    ];
  }
  ...
  return [
    ...
    { key: "p95", label: "P95", helpKey: "p95", render: (stage) => `${stage.p95_latency_ms}ms` },
    { key: "ttft", label: "TTFT", helpKey: "ttft", render: (stage) => `${stage.ttft_ms}ms` },
    { key: "tps", label: "Out TPS", helpKey: "out_tps", render: (stage) => stage.tps || "-" },
    { key: "tokens", label: "Token/s", helpKey: "token_s", render: (stage) => stage.total_tokens },
